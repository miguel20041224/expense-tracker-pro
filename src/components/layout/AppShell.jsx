...
export function AppShell({ children, activeTab, onTabChange, alertCount = 0 }) {
  return (
    <div className="min-h-screen bg-surface">
      <div className="pointer-events-none fixed inset-0 overflow-hidden" aria-hidden>
        <div className="absolute -top-32 left-1/2 h-96 w-96 -translate-x-1/2 rounded-full bg-accent/8 blur-3xl" />
        <div className="absolute top-1/3 -right-24 h-72 w-72 rounded-full bg-income/6 blur-3xl" />
        <div className="absolute bottom-0 left-0 h-64 w-64 rounded-full bg-savings/5 blur-3xl" />
      </div>
